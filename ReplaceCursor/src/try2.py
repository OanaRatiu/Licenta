...
def main():
    ...
    # text = "Try and hold your hande in the same position for 2 seconds..."
    ...
    controller.add_listener(sickenss_listener)
    time.sleep(2)
    controller.remove_listener(sickenss_listener)

    # text = "You can start using the app..."
    # ttk.Label(mainframe, text=text).grid(column=3, row=1, sticky=W)
    sickenss_value = get_sickness(sickenss_listener.fuzzy_inputs)
    if sickenss_value <= 4:
